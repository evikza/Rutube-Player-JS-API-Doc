Дополненная документация по встраиванию видео (плеера) и работа с Rutube Player API.
...
| player:setVolume | ```data: { volume: 0.2 }``` | Установить значение громкости от **0** до **1**. |
| player:setSkinColor | ```{ color: "5e35b1" }``` | Изменить цвета интерфейса плеера (HEX-модель) |
| player:changeQuality | ```data: { quality: "240" }``` | Изменить качество видео. Вызывайте данную команду после получения списка качеств у видео — событие ```player:qualityList``` |
...
| player:setCaption | ```data: { current: CAPTION_ID, enabled: true | false }``` | Установить и вывести субтитры. Список и идентификатор субтитров можно получить при вызове события ```player:playOptionsLoaded```. Ключ ```enabled``` принимает значение (boolean) в зависимости от состояния отображения субтитров в видео ⚠️🚦 |
